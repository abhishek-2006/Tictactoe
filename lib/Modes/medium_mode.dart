...
// Mode Color for Dialog (Medium Mode Orange)
const Color _kMediumModeColor = Color(0xFFFF9800);
...
class MediumModeState extends State<MediumMode> with SingleTickerProviderStateMixin {
  ...
  final SoundManager _soundManager = SoundManager(); // New dependency
  ...
  void _runCpuTurn() {
    ...
    if (emptyCells.isNotEmpty) {
      // Medium Mode Logic: 60% chance of optimal move (Minimax level 3 search), 40% random
      List<int> move;

      if (_random.nextDouble() < 0.6) {
        // Placeholder/Simplified Minimax: 60% chance of a good move
        move = _getBestMove('O', depth: 3);
      } else {
        // 40% chance: Random move
        move = emptyCells[_random.nextInt(emptyCells.length)];
      }
  ...
  // Placeholder/Simplified Minimax function (ensure it matches the original file's implementation logic if it was more advanced)
  List<int> _getBestMove(String player, {int depth = 1}) {
    ...
    // Default to a random move if no strategic move is found (for the non-100% chance logic)
    List<List<int>> emptyCells = [];
    for (int r = 0; r < 3; r++) {
      for (int c = 0; c < 3; c++) {
        if (_board[r][c] == '') {
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: const Text(
            ...
            style: TextStyle(
              color: _kMediumModeColor, // Mode color
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('MEDIUM MODE (VS CPU)', style: TextStyle(color: _currentAppBarTextColor)),
